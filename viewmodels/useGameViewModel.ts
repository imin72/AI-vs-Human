...
export const useGameViewModel = () => {
  ...
  const performBackNavigation = useCallback((): boolean => {
    ...
    const confirmHomeMsg = t.common.confirm_home || "수행내용이 초기화되고 홈화면으로 이동합니다.";

    switch (stage) {
      // 1) IntroView: 뒤로가기 시 무조건 종료팝업
      case AppStage.INTRO:
        const exitMsg = t.common.confirm_exit_app || "앱을 종료하시겠습니까?";
      ...
      case AppStage.TOPIC_SELECTION:
        if (selectionPhase === 'SUBTOPIC') {
          // SUBTOPIC 단계에서 뒤로가기 -> CATEGORY 단계로
          setSelectionPhase('CATEGORY');
          setSelectedSubTopics([]);
          return true;
        }
        // CATEGORY 단계에서 뒤로가기 -> INTRO로
        setStage(AppStage.INTRO);
        return true;

      // 3) QUIZ: 뒤로가기 시 초기화/홈 이동 확인 팝업
      case AppStage.QUIZ:
      case AppStage.LOADING_QUIZ:
      ...
      // 4) RESULTS, ERROR: 뒤로가기 시 초기화/홈 이동 확인 팝업
